...
var global = new __WEBPACK_IMPORTED_MODULE_1_vuex__["a" /* default */].Store({
  state: {
    token: null,
    pageName: '',
    deviceNumber: '',
  ...
  mutations: {
    getToken: function getToken(state, token) {
      var obj = state;
      obj.token = token;
    },
    setPageName: function setPageName(state, name) {
      var obj = state;
      obj.pageName = name;
    },
    scan: function scan(state, name) {
      var obj = state;
      if (name !== '/pages/scan/main' && name !== '/pages/charge/main' && name !== '/pages/ICrecord/main') {
        wx.scanCode({
          onlyFromCamera: true, //是否只能从相机扫码，不允许从相册选择图片,
          success: function success(res) {
            var result = res.path; // 当needResult 为 1 时，扫码返回的结果
            wx.navigateTo({
              url: '/' + result
            });
          },
    ...
    setMapTab: function setMapTab(state, tab) {
      var obj = state;
      obj.tab = tab;
    }
  }
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_flyio_dist_npm_wx__ = __webpack_require__(16);
...
var request = new __WEBPACK_IMPORTED_MODULE_1_flyio_dist_npm_wx___default.a();

request.interceptors.request.use(function (request) {
    //给所有请求添加自定义header
    request.headers["Authorization"] = "Bearer " + __WEBPACK_IMPORTED_MODULE_0__global__["a" /* default */].state.token;
